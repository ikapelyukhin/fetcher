...
module Fetcher
  ...
  class Request < Typhoeus::Request
    def initialize(logger, target_dir, url, *args)
      ...
      raise Fetcher::Exception, 'No file name in the URL' if basename == '/' || basename.empty?

      @target_file = File.join(target_dir, uri.host, uri.path)

      super(url, *args)
      @logger = logger
      @on_complete = [method(:on_complete_cb)]
      @on_body = [method(:on_body_cb)]
    end
    ...
    def on_body_cb(chunk, _response)
    ...
    def on_complete_cb(response)
      ...
      if response.success?
        ...
        if response.code != 0
          @logger.error(format('%s: request failed with code %s', url, response.code))
        else
          @logger.error(format('%s: %s', url, response.return_message))
        end
      end
    rescue StandardError => e
      @logger.error(format('%s: %s', url, e))
    ensure
      cleanup_temp_file
    end
  end
